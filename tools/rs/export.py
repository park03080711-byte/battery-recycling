"""재사용 장면 렌더 결과를 사이트용으로 내보내기: 장면 WebP 2종, 단계 마스크 PNG, 좌표 JSON."""
import sys, os, json
from PIL import Image
src, dst, js = sys.argv[1], sys.argv[2], sys.argv[3]
os.makedirs(dst, exist_ok=True)
im = Image.open(f"{src}/yard.png").convert("RGBA")
for w in (2800, 1600):
    r = im.resize((w, round(w * im.height / im.width)), Image.LANCZOS) if w != im.width else im
    r.save(f"{dst}/yard-{w}.webp", "WEBP", quality=86, method=6)
for g in ["s1", "s2", "d1", "d2", "d3", "d4"]:
